...
Når du vil utforske alternative grensesnitt for en valgt deepening-kandidat. Basert på «Design It Twice» (Ousterhout) — den første idéen din er sjelden den beste. Bruker vokabularet i [SKILL.md](SKILL.md) — **modul**, **grensesnitt**, **skjøt**, **adapter**, **gjennomslag**.
...
- Hvilke krav et nytt grensesnitt må tilfredsstille (hentes fra `.grill/CONTEXT.md` og relevante `.grill/adr/`).
...
Lag minst tre alternativer, hvert med en distinkt designtvang. Bruk vokabularet fra [SKILL.md](SKILL.md) og domenespråket i `.grill/GLOSSARY.md` så ting navngis konsistent.
...
3. Hva implementasjonen skjuler bak skjøten.
...
Presenter alternativene sekvensielt så de kan fordøyes ett for ett, og sammenlign dem i prosa. Kontraster på **dybde** (gjennomslag ved grensesnittet), **lokalitet** (hvor endring samler seg) og **skjøt-plassering**.

Gi til slutt din egen anbefaling: hvilket design som er sterkest og hvorfor. Passer elementer fra ulike alternativer godt sammen, foreslå en hybrid. Vær tydelig — gjesten vil ha et sterkt råd, ikke en meny. Det valgte grensesnittet og begrunnelsen skrives til `.grill/CONTEXT.md`; er beslutningen vanskelig å reversere, skriv ADR i `.grill/adr/`.
