...
4. Planlegg event-kontrakt (topic-navn, key, felter, `@event_name`). Kontrakten er et domenevalg — loggfør den som ADR i `.grill/adr/` og oppdater `.grill/CONTEXT.md` hvis hendelsen blir en del av domenespråket (jf. `/grill-with-docs`).
...
Breaking event-endringer er et koordineringsproblem med konsumerende team — samme disiplin som API-versjonering (`/api-design`). Loggfør beslutningen som ADR i `.grill/adr/`.
