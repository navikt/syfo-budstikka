...
4. Mangler malen eller linjen → repoet har ikke prosjekttilknytning konfigurert → hopp over hele prosjektflyten uten å feile

## Minimumssekvens for prosjekttilknytning

0. **Auth-preflight før prosjektkall**
   Kjør `gh auth status` og se etter `project`-scope før du bruker `gh project ...`.
   - Mangler `project`-scope: stopp før første `gh project`-kall og presenter denne meldingen ordrett:
   ...
   - Velger brukeren å hoppe over: skip prosjektsteget stille og fortsett resten av issue-flyten.
1. **Finn prosjekt-ID**
...
3. **Oppdag felter dynamisk**
   Bruk `gh project field-list NUMBER --owner OWNER --format json`. Finn felter som `Status` og evt. `Type` etter navn, og les opsjoner fra responsen. Felt- og option-ID-er er prosjektspesifikke — hardkod aldri verdier.
...
Finnes ikke et felt i boardet, skal kun det feltet hoppes over — ikke hele opprettelsen.

## Statusovergang når arbeid starter

Når brukeren velger et issue og ber om å starte arbeid:

0. Kjør samme auth-preflight som over før første `gh project`-kall. Hopper brukeren over, skip prosjektsteget stille og fortsett.
1. Kjør samme runtime-discovery av `projects:`-linjen.
2. Finn eksisterende item via `gh project item-list NUMBER --owner OWNER --format json` og match på issue-URL. `item-list` returnerer maks 30 items som standard; bruk `--limit 100` for store prosjekter (eller paginer ved behov).
3. Ligger ikke issuet i prosjektet, kjør `item-add` først.
4. Hent feltmetadata på nytt via `field-list`.
5. Oppdater `Status` ved å matche mot kjente synonymer først: `In Progress`, `Doing`, `Påbegynt`, `I arbeid`, `Under arbeid`. Ingen treff → velg nærmeste aktive / ikke-`Done`-opsjon i feltet.

Ikke anta at alle board bruker samme statusnavn. Match mot opsjonene i det konkrete prosjektet.
...
- `gh issue close` eller PR med `Closes #...` flytter i de fleste Projects V2-oppsett issuet til `Done` automatisk; ikke bygg egen Done-logikk med mindre boardet krever det. Forutsetter at prosjektets innebygde `Item closed`-workflow er aktivert (default på i nye prosjekter, men kan ha blitt deaktivert under Settings → Workflows).
