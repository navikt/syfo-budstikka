...
Start only after the user has explicitly selected Wayfinder in the current
conversation. If it was merely relevant or recommended, return to the caller
without charting, claiming, or changing tracker state.
...
authorization. Another session treats an assignment as active unless the
...
- **Research** (AFK): Reading documentation, third-party APIs, or local resources like knowledge bases to surface a fact a decision waits on. Resolved by the internal Researcher agent through the agent task tool. Use when knowledge outside the current working directory is required.
- **Prototype** (HITL): Raise the fidelity of the discussion by making a cheap, rough, concrete artifact to react to — an outline, a rough take, a stub, or UI/logic code via the /prototype skill. Links the prototype as an asset. Use when "how should it look" or "how should it behave" is the key question.
- **Grilling** (HITL): Conversation. The default case. Use /grilling. Use /domain-modeling only when the repository's domain policy qualifies the result for durable documentation and the user selects that route.
...
3. Resolve it — **zoom as needed**: fetch the full body of any related or closed ticket on demand; invoke the skills the `## Notes` block names. If in doubt, use `/grilling`; use `/domain-modeling` only through its durable-documentation gate.
