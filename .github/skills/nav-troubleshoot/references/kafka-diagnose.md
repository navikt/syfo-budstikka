...
Diagnostiske trær for NAIS Kafka-konsumenter i dette Ktor-backendet (`no.nav.syfo`). **Detekter faktisk stack først** — appen kan bruke plain Apache Kafka clients (Kotlin) eller Rapids & Rivers. Diagnosen må matche det kodestien faktisk kjører.
...
├── SSL / oppkobling?
│   ├── "SSL handshake failed" → sjekk at `KAFKA_TRUSTSTORE_PATH` / `KAFKA_KEYSTORE_PATH` er mountet og lest
...
| Konsument prosesserer ingenting, ingen feil | Feil `group.id` / topic-navn | Sjekk NAIS-manifest og Ktor-config |
...
- Sjekk at River bruker samme topic(s) som producer skriver til (rapids-topic er ofte team-delt).
...
- Auth-feil ved nedstrøms-kall fra konsumenten → [auth-diagnose.md](./auth-diagnose.md)
- Fikse-disiplin (replay av fanget record gjennom handleren) → `/diagnosing-bugs`
