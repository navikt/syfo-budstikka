...
1. **Existing README** — preserve manual content, Slack channels, wiki links and stable operational tips that still hold. The generated Ktor scaffold (links to ktor.io, an empty "Features" table, the `./gradlew run` table) is not manual content — it is to be replaced.
