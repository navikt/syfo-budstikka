...
3. **One command to run.** Use Gradle the way the repository already does — a throwaway `fun main()` run via a `JavaExec` task or from the IDE, or a `@Test` that drives the model. The user should never have to remember a path.
4. **No persistence by default.** State lives in memory. Persistence is what the spike *checks*, not something it should rest on. If you must hit a database, use an in-memory `H2`/Testcontainers instance or a local schema named something that shouts `SPIKE — delete me`. Never a shared or real database.
5. **No auth, no polish.** Drop TokenX/Azure AD validation, `StatusPages`, retry, metrics, logging beyond what makes the spike *runnable*. Those are precisely the things whose shape you are testing, not something the spike should implement.
...
- **The answer is the only thing that is stored.** The task scope goes to the
  ...
  the documented path and wait for the user's decision before `/domain-modeling`
...
- **Do not write tests for the spike.** If the spike needs tests it is no longer a spike — then it should be lifted into real code and tested there.
