...
4. **No persistence by default.** State lives in memory. Persistence is what the spike *checks*, never something it rests on. If you must hit a database, use a throwaway Testcontainers Postgres instance or a local schema named something that shouts `SPIKE — delete me`. Never a shared or real database.
