...
class FerdigstillMatchingIntegrationTest :
    FunSpec({
        ...
        ) = EffectuateDecision(TransactionRunnerImpl(fixture.database), inbox, deliveries, clock)
        ...
        test("ARBEIDSGIVERVARSEL stored CREATE without recoverable external id is terminally invalid") {
            val (inbox, deliveries) = repositories()
            val effectuate = effectuator(inbox, deliveries)
            val reference = "invalid-create-external-id"
            val create =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000827"),
                    reference,
                    ArbeidsgivervarselCreate(
                        orgnummer = TEST_ORGNUMMER,
                        recipient = AltinnResource(AltinnResourceId.DIALOGMOETE),
            ...
            val inactivate =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000828"),
                    reference,
                    ArbeidsgivervarselInactivate(reference, TEST_ORGNUMMER),
                )
            ...
            effectuate.effectuate(create, createDecision(create)) shouldBe EffectuationResult.Completed
            fixture.database.transact {
                DeliveryTable.update({ DeliveryTable.reference eq reference }) {
                    it[inboxEventId] = null
                    it[createExternalId] = null
                }
            }
        ...
        test("FERDIGSTILL waits while a matching READY CREATE delivery is pending and derives exactly one INAKTIVER after SENT") {
            val (inbox, deliveries) = repositories()
            val clock = MutableClock(Instant.parse("2026-08-18T12:00:00Z"))
            val effectuate = effectuator(inbox, deliveries, clock)
            val reference = "ready-create-state-ref"
            val create =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000829"),
                    reference,
                    BrukervarselCreate(TEST_SYKMELDT, Varseltype.BESKJED, "ready create"),
                )
            val inactivate =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000830"),
                    reference,
                    BrukervarselInactivate(reference, TEST_SYKMELDT),
                )
            ...
            fixture.database.transact {
                val row = InboxMessageTable.selectAll().where { InboxMessageTable.eventId eq inactivate.eventId }.single()
                row[InboxMessageTable.state] shouldBe "WAIT"
                row[InboxMessageTable.waitReason] shouldBe "AWAITING_MATCHING_CREATE_SENT"
                row[InboxMessageTable.nextAttemptTime] shouldBe clock.current + FERDIGSTILL_CREATE_SENT_RECHECK_DELAY
            }
            inactivateRows(reference).shouldBeEmpty()

            val createDelivery =
                deliveries
                    .claim(limit = 1, lease = lease, maxAttempts = 10, channels = setOf(Channel.BRUKERVARSEL))
                    .single()
            deliveries.markSent(createDelivery.id) shouldBe true
            fixture.database.transact {
            ...
            inbox.claim(limit = 10, lease = lease, maxAttempts = 10).map { it.eventId } shouldBe listOf(inactivate.eventId)
        ...
        test("FERDIGSTILL waits without creating INAKTIVER while the matching CREATE delivery is CLAIMED") {
            val (inbox, deliveries) = repositories()
            val clock = MutableClock(Instant.parse("2026-08-18T12:05:00Z"))
            val effectuate = effectuator(inbox, deliveries, clock)
            val reference = "claimed-create-state-ref"
            val create =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000839"),
                    reference,
                    BrukervarselCreate(TEST_SYKMELDT, Varseltype.BESKJED, "claimed create"),
                )
            val inactivate =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000840"),
                    reference,
                    BrukervarselInactivate(reference, TEST_SYKMELDT),
                )
            ...
            saveAndClaim(inbox, create)
            effectuate.effectuate(create, createDecision(create)) shouldBe EffectuationResult.Completed
            deliveries.claim(limit = 1, lease = lease, maxAttempts = 10, channels = setOf(Channel.BRUKERVARSEL)).single()
            saveAndClaim(inbox, inactivate)
        ...
        test("FERDIGSTILL processes terminally without close when the matching CREATE delivery is FAILED") {
            val (inbox, deliveries) = repositories()
            val effectuate = effectuator(inbox, deliveries)
            val reference = "failed-create-state-ref"
            val create =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000841"),
                    reference,
                    BrukervarselCreate(TEST_SYKMELDT, Varseltype.BESKJED, "failed create"),
                )
            val inactivate =
                InboxMessage(
                    UUID.fromString("00000000-0000-0000-0000-000000000842"),
                    reference,
                    BrukervarselInactivate(reference, TEST_SYKMELDT),
                )
            ...
            val createDelivery =
                deliveries
                    .claim(limit = 1, lease = lease, maxAttempts = 10, channels = setOf(Channel.BRUKERVARSEL))
                    .single()
            deliveries.markFailed(createDelivery.id, "Downstream rejected create") shouldBe true
            saveAndClaim(inbox, inactivate)
