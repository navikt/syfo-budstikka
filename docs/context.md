...
## Ledervarsel-kanal-beslutninger (issue #108; se ADR 0008, 0009)
- B61: LEDERVARSEL-`oppgavetype` = LUKKET, budstikka-eid enum i kontraktlib (ADR 0008), IKKE opak streng. Dinesykmeldte-kontrakten (`OpprettHendelse.oppgavetype`) er PÅKREVD, inngår i konsumentens PK `(id, oppgavetype)` og styrer UI-gruppering. VALGT lukket enum framfor åpen value class/streng fordi B23 (anti-corruption) veier tyngst: en opak streng hvis gyldige verdier ER dinesykmeldtes vokabular tvinger produsentene til å kjenne dinesykmeldtes API i tillegg til budstikkas → abstraksjonslekkasje (art-forskjell), mens B1-bekymringen (budstikka navngir domenenære verdier) kun er en onboarding-kostnad (grad) og er den milde koblingen B30 alt sanksjonerte (navngi ≠ forgrene; `when(oppgavetype)` finnes aldri). Konsistent med at HVERT kategoriserings-felt i kontrakten alt er budstikka-eid enum (`Tag`/`AltinnResourceId`/`Varseltype`/`ArbeidsgiverMeldingstype`/`SendingWindow`/`DistributionType`). NAVNGIVNING (i): case-navn = budstikkas domeneord + eksplisitt `wireValue` = dinesykmeldtes eksakte streng (frikoblet identifikator↔wire). #108 leverer maskineri + ÉN representativ verdi (`DIALOGMOTE_INNKALLING`); resten (`DIALOGMOTE_AVLYSNING/ENDRING/REFERAT/SVAR_BEHOV`, `OPPFOLGINGSPLAN_TIL_GODKJENNING/PAAMINNELSE`) fylles ADDITIVT ved onboarding (B36), kun LEDERVARSEL-verdier — ikke hele `DineSykmeldteHendelseType`-katalogen på forhånd. Kostnad tatt bevisst: budstikka i release-stien per ny oppgavetype (samme slags skatt som merkelapp B30; ny oppgavetype krever dinesykmeldte-release uansett → koordinert release er normalen).
- B62: LEDERVARSEL-KANALEN ER RENT IN-APP (ADR 0009; KORRIGERER B25). Nedstrøms `team-esyfo.dinesykmeldte-hendelser-v2` → `dinesykmeldte-backend.handleHendelse` gjør KUN et DB-insert (aktivitetsvarsel i Dine Sykmeldte-oversikten); INGEN ekstern push (SMS/e-post). Ekstern varsling til nærmeste leder kommer i esyfovarsel fra ARBEIDSGIVERNOTIFIKASJON (fager: Sak/Kalenderavtale m/`ledersEpost`), dvs. budstikkas ARBEIDSGIVERVARSEL-kanal med `NarmesteLeder(sykmeldt)`-mottaker (B32, #24) — IKKE fra Dine Sykmeldte. KONSEKVENS: (a) `externalVarsling` FJERNES fra `LedervarselCreate` v1 (falsk affordance, B40) — vil konsument også e-poste lederen, sendes en egen ARBEIDSGIVERVARSEL-dispatch (B6: flere kanaler → flere hendelser). (b) `sendingWindow` BEHOLDES (budstikka kan gate in-app-visning), men default flippes til `ONGOING` (LØPENDE). (c) KORRIGERER B25: «ledervarsel m/ekstern» STRYKES fra listen over eksternbærende leveranser (bygde på feil antakelse); ledervarsel er ren in-app → default `ONGOING` som microfrontend/in-app. v1-kontrakten er ikke i prod → fjerning av felt er ikke breaking; additivt å legge til igjen ved reell ekstern-ledervarsel-bærer.
