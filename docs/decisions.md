# Decision register — syfo-budstikka

> Canonical definitions for the `Bnn` references used by code comments, ADRs,
> and topic documents. The original wording is retained to preserve decision
> intent. Superseded decisions are marked individually; use
> [`context.md`](context.md) for the current mental model and navigation.
> Dated status and legacy-system background are preserved separately in
...
## Data-model decisions (see `docs/datamodell.md`)
...
- B61: HYDRERT INBOX — FULL PARSE VED INGEST, EVENTID KUN I HEADER (SUPERSEDER ADR 0002; se ADR 0008; REVERSERER B54s
  ...
  matching if the still-open hold-placement follow-up below chooses inbox hold).
  `recipient`/`channel`/`operation` løftes IKKE (utledbare fra `content` `partitionKey`/`type`,
  brukes kun til avgrensning innenfor et reference-treff → ingen spekulativ schema; ytterligere match-kolonner legges
  til KUN hvis that follow-up chooses inbox hold). `ignoreUnknownKeys=true` beholdes → ukjente felt forsvinner BEVISST (kontrakt =
  ...
  vs outbox) forblir ÅPEN i B61 men er nå AVBLOKKERT — hydrert inbox gjør både outbox-hold-med-`CANCELLED`
...
## Ferdigstill decisions (see `docs/ferdigstill.md`)

- B19: FERDIGSTILL er kanal-eksplisitt (referanse pr. kanal/mottaker, symmetrisk med B6). Matching på (referanse,
  recipient_id, kanal). Lukking gjenbruker delivery-maskineriet: en `delivery`-rad med `operation=INACTIVATE` og samme
...
## Contract and channel-DTO decisions (see `docs/kontrakt.md`)
...
## Worker and processing decisions

- B27: Worker-topologi = ÉN generisk dyp modul + tynn `ChannelHandler`-seam (dispatch på `kanal`). Det generiske
  ...
  side-effekt uten distribuert tx å kompensere; brev-fallback + OPPRETT- og FERDIGSTILL-hendelser er allerede egne rader +
  ...
  SAMME deployment (config, ikke rearkitektur). `kanal`-kolonnen + `ChannelHandler` ER sømmen som muliggjør senere
...
- B28: Beslutnings-workeren struktureres som FUNCTIONAL CORE / IMPERATIVE SHELL i tre sammenhengende steg i samme
  ...
  Datainnhenting er BETINGET på hendelsestype (start enkelt; evt. `Databehov`-deklarasjon per `ChannelHandler` senere).
  Fallgruve unngått bevisst: stopp på TRE steg — ikke N tynne pass-through-lag.

## Arbeidsgivervarsel decisions (see `docs/kontrakt.md`)
...
## Inactivate decisions (see `docs/ferdigstill.md` and `docs/kontrakt.md`)
...
- B39: INAKTIVER-OPERASJON AVLEDES FRA LAGRET OPPRETT-RAD, ikke fra hendelsen. FERDIGSTILL-hendelsen er THIN
  ...
  lukkeparametrene fra OPPRETT-raden onto `INACTIVATE`-delivery (`meldingstype`, sti NL/Altinn, `ekstern_respons_id`,
...
## Retention and GDPR decisions (see `docs/datamodell.md`)
...
## Migration decisions (see `docs/migrering.md`)
...
## Technology decisions (see `docs/teknologi.md`)
...
## Observability decisions (see `docs/datamodell.md`, `docs/flyt.md`, and B17)
...
- B47: [DELVIS SUPERSEDERT av B57 — norske metrikknavn er erstattet; de utsatte målingene er fortsatt referanse. Se ADR 0007] METRIKK-KATALOG. Micrometer → Prometheus; `snake_case`, `_total`/`_seconds`-suffiks, lav-kardinalitets labels
...
## Local testing and e2e decisions (see `docs/teststrategi.md`)
...
- B63: BUILD CONTAINERS WITH JIB, NOT A DOCKERFILE (development POC; see ADR 0010).
  Use a plain `jib { }` block in `build.gradle.kts`, not Ktor `docker { }` tasks,
  avoiding the Gradle 10 deprecation from `setupJibLocal` and its JRE validation.
  Configure the Chainguard base in `from.image` through `docker://` plus a
  `docker pull`, because Jib fails on Chainguard's verified OCI 1.1
  `artifactType` manifest. Deployment runs `nais/login`, `./gradlew jib`
  (direct push), reads `build/jib-image.digest`, then runs `nais/attest-sign`,
  preserving the SLSA/signing behavior from `nais/docker-build-push`. Derive
  the image path from the `nais/login` registry output, tag it
  `YYYY.MM.DD-HH.mm-<sha>`, and pin third-party Actions by SHA. REVISED BY ADR
  0010: the pull-request and merge gates run build, lint, and tests without OIDC
  or registry credentials; Jib build, push, and signing happen only in the
  trusted `main` deployment. The `Dockerfile` is removed and no parallel
...
## Authentication and ACL decisions (area 5; see ADR 0009)
