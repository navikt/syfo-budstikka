# 0006: Opt-in gating av fulle e2e-tester (egen Gradle-task + Kotest-tag)
...
wiret inn via en wiring-søm (`configureApplication(overrides)`). Samme substrat driver både de
automatiske e2e-specene og et kjørbart lokalt løp (`./gradlew runLocal`).

Disse full-boot-testene starter to Docker-containere og venter på at Kafka-konsumenten poller og
workerne prosesserer asynkront (`eventually`), og tar tiendetalls sekunder til titalls sekunder per
kjøring. B53 forutsatte «automatiske e2e-specer» uten å ta stilling til NÅR de kjører. Kravet fra
...
2. **Env-var-switch i én test-task.** Én `test`-task som betinget inkluderer e2e ut fra en
   miljøvariabel. Vraket: skjuler hvilke tester som faktisk kjørte bak runtime-tilstand, og gjør
...
3. **`./gradlew e2eTest` kjører KUN e2e.** En egen `Test`-task med `kotest.tags=E2E`, egen
   ...
   IKKE wiret inn i `check`. Brukes lokalt og i en egen manuell/nattlig CI-jobb.
...
Wiring-sømmen som muliggjør fake-injeksjon: `ktor.di.conflictPolicy = "OverridePrevious"` settes
...
Se mekanismene 1–3 i Kontekst. Env-var-switch (2) ble vraket fordi den gjør default-gaten
ikke-deterministisk og skjuler hva som kjørte. Eget source-set/modul (3) ble vraket fordi det
